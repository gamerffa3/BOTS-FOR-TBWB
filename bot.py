# bot.py - Complete Discord Bot with AI, Moderation, Voice & Full Control
import os
import discord
...
def ask_ai(prompt, model=None):
    ...
    try:
        ...
        data = {
            "model": model,
            "messages": [
                {"role": "system", "content": f"You are {BOT_NAME}, a helpful AI assistant created by {CREATOR}. You are friendly, professional, and knowledgeable. Answer in clear, concise language. Always be helpful and respectful."},
...
async def vc_join_owner(ctx, channel_id: int):
    """Owner: Force join VC"""
    if not is_owner(ctx.author):
        await ctx.send("❌ Only bot owner can use this!")
        return
    
    await join_vc(ctx, channel_id)
...
async def set_ai_model(ctx, model: str):
    ...
    save_config()
    await ctx.send(f"✅ AI model changed to: **{model}**")
